"""
anomaly_detector_Rucu.py - Project script (example).
...
- Each row represents a patient visit with four key measurements:
  - age_years: The patient's age in years.
  - height_inches: The patient's height in inches.
  -body_weight: The patient's weight in lbs.
...
Paths (relative to repo root)
    ...
    INPUT FILE: data/clinic_data_Rucu.csv
    OUTPUT FILE: artifacts/anomalies_Rucu.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.anomaly_detector_Rucu
...
DATA_FILE: Final[Path] = DATA_DIR / "clinic_data_Rucu.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "anomalies_Rucu.csv"
...
def main() -> None:
    ...
    LOG.info("Studying children's ages, heights, weight and BMI to find anomalies...")

    # x is age in years, so 10 is the upper limit for kids
    MAX_REASONABLE_X_VALUE: Final[float] = 10.0

    # y is height in inches, so maybe 6 feet (72 inches) is a reasonable upper limit
    MAX_REASONABLE_Y_VALUE: Final[float] = 73.0

    # a is weight in lbs, so  180 lbs is a reasonable weight
    MAX_REASONABLE_A_VALUE: Final[float] = 180

    # b is BMI , so 24.21 is a reasonable BMI
    MAX_REASONABLE_B_VALUE: Final[float] = 25.00
    ...
    LOG.info(f"MAX_REASONABLE_A_VALUE: {MAX_REASONABLE_A_VALUE} in lbs")
    LOG.info(f"MAX_REASONABLE_B_VALUE: {MAX_REASONABLE_B_VALUE}")
    ...
    anomalies_df: pl.DataFrame = df.filter(
        ...
        | (pl.col("weight_lb") >= MAX_REASONABLE_A_VALUE)
        | (pl.col("BMI") >= MAX_REASONABLE_B_VALUE)
    ).with_columns(
        pl.when((pl.col("age_years") >= 0) & (pl.col("age_years") <= 4))
        .then(pl.lit("0-4"))
        .when((pl.col("age_years") >= 5) & (pl.col("age_years") <= 8))
        .then(pl.lit("5-8"))
        .when((pl.col("age_years") >= 9) & (pl.col("age_years") <= 12))
        .then(pl.lit("9-12"))
        .otherwise(pl.lit("> than 12 years"))
        .alias("age_range")
    )
    # Count anomalies per range
    range_counts = anomalies_df.group_by("age_range").agg(
        pl.count().alias("number_of_anomalies_per_range")
    )
    # Join counts back into anomalies_df
    anomalies_df = anomalies_df.join(range_counts, on="age_range", how="left")

    ## count anomalies per age range
    LOG.info(f"Count of anomalies found: {anomalies_df.height}")
    # -- Adding the count column into anomalies_df
    # ----------------------------------------------------
    ...
    LOG.info("Modified Project with anomalies colun and range count fields")
    LOG.info("========================")
    LOG.info("END main()")
